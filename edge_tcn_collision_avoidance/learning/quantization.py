...
def intermediate_validation(
        xtrain: np.ndarray[np.uint8 | np.float32],
        ytrain: np.ndarray[bool],
        xvalid: np.ndarray[np.uint8 | np.float32],
    ...
    metrics_train = good.compute_detection_metrics(
        ytrain, ysoft_train, pos_weight=1.0)  # TEMPORARY WEIGHT
    metrics_valid = good.compute_detection_metrics(
        yvalid, ysoft_valid, pos_weight=1.0)  # TEMPORARY WEIGHT
    ...
    # plt.hist(yout_train[ytrain], bins=100, color='r', alpha=0.5)
    # plt.hist(yout_train[~ytrain], bins=100, color='g', alpha=0.5)
    # plt.show()
    # plt.hist(yout_valid[yvalid], bins=25, color='r', alpha=0.5)
    # plt.hist(yout_valid[~yvalid], bins=25, color='g', alpha=0.5)
    # plt.show()
    ###########################################################################
    ###########################################################################
    ###########################################################################

    return metrics_train, metrics_valid, ysoft_train, ysoft_valid
...
def quantlib_flow(
    xtrain: np.ndarray[np.uint8],
    ytrain: np.ndarray[bool],
    xvalid: np.ndarray[np.uint8],
    yvalid: np.ndarray[bool],
    model: torch.nn.Module,
    do_qat: bool,
    num_epochs_qat: int,
    ...
    if do_qat:
        model_fq_rounded, history, ysoft_train_qat, ysoft_valid_qat = \
            learn.do_training(
                ...
                model_fq_rounded, num_epochs=num_epochs_qat,
            )
    ...
    scale_last = named_modules_list[-1][1]._buffers['scale'].item()
    ...
    xvalid = xvalid.astype(np.float32) / 255.0
    ...
    if export:
        single_input = xtrain[0]  # to trace the `nn.Module` (here, just one x)
        do_onnx_export(
            model=model_tq,
